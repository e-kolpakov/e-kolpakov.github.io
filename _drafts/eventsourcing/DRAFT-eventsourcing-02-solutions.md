...
* During node *crash*, the system is allowed to loose some of the requests for a brief period of time - 3-5 minutes 
...
Here's the scenario our system needed to support. We have two customers - let's call them **Alice** and **Bob**, as 
usual. The orders they would place will be allocated to the same capacity pool - let's call it _6pm slot_ (see 
[the previous post][post-problem-analysis] for definitions). The pool can only accept one order. The real 
time (aka wall clock) sequence of their interactions with the system and outcomes are as follows:
...
Bob's order arrive to the server earlier and is accepted, _6PM slot_ is now out of capacity.
Bob sees confirmation screen.
...
One thing to add here is that we only want _one_ order placed, but we _don't care which one_.
...
2. **Single-object updates:** each update operation only touches single pool of capacity.
3. **Reads can have relaxed consistency:** it's ok to show slightly outdated view of capacity to the customers.
    There is some natural delay between the time when capacity overview is displayed (read query) and reservation is
...
This led us to a conclusion that we need a system with *strong single-object consistency* for writes and 
...
**many clients**, consistent between **all the nodes**, but **not necessarily matching the wall clock** sequence of 
the requests. The most permissive consistency model that matches has these features 
...
[post-problem-analysis]: {% post_url design/2020-06-27-eventsourcing-01-problem %}#analyzing-the-problem
...
presented the implementation proposal at the design review session - as a response to an unspoken, but unanimous 
...
* Finally, the [existing system][post-problem-existing-system] was built with this approach in mind, and a few 
    significant issues and outages was attributed to consistency/latency/throughput issues arising from the use of this
    architecture. 
...
[post-problem-further-analysis]: {% post_url design/2020-06-27-eventsourcing-01-problem %}#further-analysis
[post-problem-existing-system]: {% post_url design/2020-06-27-eventsourcing-01-problem %}#existing-solution
...
The general idea is to put a "version number" onto a record, and at the DB side only perform a write/update when the 
version number sent by the application matches the version number in the DB and if not - reject the write, assuming 
the app will re-read the record, re-apply the modification logic and retry the update. This technique is very popular,
as it allows enforcing strong(er) consistency in a relatively straightforward way.

This approach works well if majority of the updates are predicted to not face a conflict - if no conflict happens, the 
overhead compared to "don't even bother about consistency" (aka Last Writer Wins) is marginal. However, the resource 
consumption and latency virtually doubles in case of a conflict, and in highly concurrent scenarios single request might
face more (sometimes _much_ more) than one conflict.

This is why we rejected this approach - our **back of the envelope calculation showed that even at 3x current rate, we 
would face latency spikes beyond our SLA**. Moreover, certain capacity "types" we needed to model and support essentially 
needed to be represented as a _single_ record[^3] - which would cause many conflicts right away.
...
the concurrency model is "pessimistic" - all processes assume that a conflict will happen, and always pay some price 
(acquire a lock, wait on a semaphore, etc.) to prevent it.

The OS-level primitives are unable to "synchronize" access between different nodes of a distributed systems. However,
...
1. Theoretical: at the time of designing the system, we were not aware of Redlock, so we thought only a single-node 
    ...
    at one layer the application instances would need to wait acquiring the locks ("domain" contention) and deeper down 
    the infrastructure stack, different Redis clients will also compete for Redis' execution engine to handle their 
    requests ("shared resource" contention).
2. Practical: the Redis instance that we had in production at that moment had some weird configuration or networking 
    issue. During an unrelated project, where Redis as used as a read-through cache I noticed that **reading a single 
    "record" from Redis was slower than from the source of truth**. 
...
*Personal opinion*: I'm with Kleppmann on this (but I'm not an expert on the topic and I may be biased - I've read 
his book). If I were designing this system now, knowing about Redlock, I think I'd still reject the distributed 
locking approach.
...
The idea here is simple - when request comes, don't do anything but put it into a dedicated queue. Have _one_ 
...
way to achieve "deduplication" and consistency in the organization - there were a couple of systems that relied on this
mechanism to ensure "exactly once" semantics - albeit on a less critical and non-time sensitive tasks.
...
not autmoatically imply "response" to the message, it would be necessary to build the "request-response" communication
...
eliminate race conditions between the nodes. Furthermore, **access to the entity state will be protected from 
concurrent access**, thus making entity business logic running in a "single threaded" fashion. This way, we will ensure
the total order of operations by passing all the updates through a single entity instance.

**Availability:** The paragraph above sounds as if we're re-introducing a single point of failure - i.e. if the node 
running an entity is down, the entity will not be available anymore. This is _partially_ true. First, the entities will 
be distributed between many nodes[^4], so if any node is down, only some of the entities will become unavailable. 
Second, the system will detect such cases, and redistribute the affected entities among healthy nodes automatically - 
...
interact with the database once - to save the update.

**Persistence:** Entities will use eventsourcing to save the changes. - so instead of **overwriting the state** on each 
update, the entity will only **append an event** that caused the change. This will happen before actual in-memory state
...
mathematically proven and there's no way around them. However, being a theorems they have a very precise and strict 
definitions of what "consistency" and "availability" actually means. By these theorems, the proposed system would be 
consistent, partition-tolerant, but _not_ available - i.e. in presence of partition we favor consistency 
...
So, for the practical purposes, there's a **"time horizon" of the availability** - how much it takes a system to recover 
from partial or total failure. For the people-facing system such as ours, anything that recovers in less than 200ms is 
virtually indistinguishable from perfectly available system, and recovery times of up to a few seconds are acceptable
from time to time.
...
In the next post, we will explore the implementation of the system, including high-level architecture, concrete 
technology choices, tactical design decisions and, most importantly, use of the eventsourcing patterns and techniques 
